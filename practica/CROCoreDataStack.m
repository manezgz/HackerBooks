...
@implementation CROCoreDataStack
    ...
    if (_storeCoordinator == nil) {
        ...
        if (![_storeCoordinator addPersistentStoreWithType:NSSQLiteStoreType
            ...
            NSNotification *note = [NSNotification
                                    notificationWithName:[CROCoreDataStack persistentStoreCoordinatorErrorNotificationName]
                                    object:self
                                    userInfo:@{@"error" : err}];
...
-(void) saveWithErrorBlock: (void(^)(NSError *error))errorBlock{
    ...
    if (!_context) {
        err = [NSError errorWithDomain:@"CROCoreDataStack"
                                  code:1
                              userInfo:@{NSLocalizedDescriptionKey :
                                             @"Attempted to save a nil NSManagedObjectContext. This CROCoreDataStack has no context - probably there was an earlier error trying to access the CoreData database file."}];
        errorBlock(err);
